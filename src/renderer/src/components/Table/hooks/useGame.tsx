...
import { Facing, useCardPile, UseCardPileReturn, useDeck } from '../../Card';
...
interface UseGameReturn {
   ...
   */
  player: UseCardPileReturn;
  /**
   * Information about the dealers cards and
   * functionality to interact with them
   */
  dealer: UseCardPileReturn;
  /**
  ...
  winner: Winner;
  setup: () => void;
  startDealerRound: () => void;
  getParticipantScore: (participant: Participant) => number;
  dealCardsToParticipant: (participant: Participant, count?: number) => void;
}
...
export function useGame(): UseGameReturn {
   ...
   */
  function startDealerRound(): void {
   ...
   */
  function getParticipantScore(participant: Participant): number {
    switch (participant) {
      case 'Dealer':
   ...
   */
  function dealCardsToParticipant(participant: Participant, count = 1): void {
   ...
   */
  function setResult(outcomeText: string, winner?: Participant | null): NodeJS.Timeout {
    return setTimeout(() => {
      setState('GameOver');
