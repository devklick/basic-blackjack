...
function Table({ hide = false }: TableProps): JSX.Element {
  const game = useGame();
  ...
  const onHitClicked = (overrideWarning: boolean = false): void => {
  ...
  const onStickClicked = (overrideWarning: boolean = false): void => {
    if (game.state !== 'PlayerRound') return;
  ...
  function getHitWarning(): JSX.Element {
    return (
      <YesNoPopUp
        type="hit-on-high"
  ...
  function getStickWarning(): JSX.Element {
    return (
      <YesNoPopUp
        type="stick-on-low"
  ...
  return (
    <div className={styles.Table} style={{ display: hide ? 'none' : '' }}>
      ...
      <CardRow cardOwner={'Dealer'} cards={game.dealer.cards} effect={getCardEffect('Dealer')}>
        <SettingsButton />
      </CardRow>
      <CardRow cardOwner={'Player'} cards={game.player.cards} effect={getCardEffect('Player')} />
      <InfoHud
